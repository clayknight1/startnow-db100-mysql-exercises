...
SELECT amount, payment_date, payment_id FROM payment WHERE DATE(payment_date) IN('2005-05-25' , '2005-05-27', '2005-05-29');  
...
SELECT * FROM payment WHERE (payment_date BETWEEN '2005-05-25 00:00:00' AND '2005-05-26 00:00:01');
...
SELECT title, description, release_year, rental_rate * rental_duration AS total_rental_cost FROM film WHERE CHAR_LENGTH(description) BETWEEN '100' AND '120';
...
SELECT * FROM customer LIMIT 200 , 100;
...
SELECT title, description, special_features, length, rental_duration FROM film WHERE special_features LIKE "%Behind the Scenes%" AND rental_duration BETWEEN 5 and 7 AND length < 120 ORDER BY length DESC Limit 10;
